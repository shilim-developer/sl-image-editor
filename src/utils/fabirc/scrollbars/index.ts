...
export class Scrollbars {
  ...
  constructor(canvas: Canvas, props: ScrollbarsProps = {}) {
    // I want to write canvas._onMouseDown = this.mouseDownHandler.bind(this), but it doesn't work. [百撕不得骑姐]
    canvas.__onMouseDown = this.mouseDownHandler.bind(this);
    canvas._onMouseMove = this.mouseMoveHandler.bind(this);
    canvas._onMouseUp = this.mouseUpHandler.bind(this);
  ...
  mouseMoveHandler(e: TPointerEvent) {
    ...
    if (!this._bar) return Canvas.prototype._onMouseMove.call(this.canvas, e);
    const p = this.canvas.getViewportPoint(e);
    const s =
  ...
  mouseUpHandler(e: TPointerEvent) {
    if (!this._bar) Canvas.prototype._onMouseUp.call(this.canvas, e);
    delete this._bar;
  }
  beforeRenderHandler() {
  ...
  dispose() {
    ...
    // @ts-expect-error: In the initialization, _onMouseMove was overridden, here it is restored
    delete this.canvas._onMouseMove;
    // @ts-expect-error: In the initialization, _onMouseUp was overridden, here it is restored
    delete this.canvas._onMouseUp;
